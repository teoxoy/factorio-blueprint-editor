...
export default function generateBeacons(
    ...
    const validBeaconPositions = U.uniqPoints(entities
        .filter(e => e.effect)
        .map(e => {
    ...
    const possibleBeaconAreas = validBeaconPositions
        .map(coord => U.range(0, BEACON_SIZE * BEACON_SIZE)
            .map(i => ({
    ...
    while (possibleBeacons.length) {
        possibleBeacons = possibleBeacons
            .sort((a, b) => {
                if (a.effectsGiven === 1 || b.effectsGiven === 1) return b.avgDistToEntities - a.avgDistToEntities
        ...
        const toRemove: IBeacon[] = beacon.collisionArea
            .reduce((acc, p) => {
                const beacons: IBeacon[] = pointToBeacons.get(U.hashPoint(p))
                if (!beacons) return acc
