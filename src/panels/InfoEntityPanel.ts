...
export class InfoEntityPanel extends Panel {
    ...
    updateVisualization(entity?: Entity) {
        ...
        for (const ingredient of FD.recipes[entity.name].ingredients) {
            InventoryContainer.createIconWithAmount(this.m_RecipeRawContainer, nextX, 0, ingredient.name, ingredient.amount)
            nextX += 36
        }
        nextX += 2
        ...
        if(entity.entityData.type == "assembling_machine") {
            ...
            if (entity.modules.length > 0) {
                for (const module of entity.modules) {
                    if(FD.items[module].effect.productivity) k_productivity += FD.items[module].effect.productivity.bonus
                    if(FD.items[module].effect.consumption) k_consumption += FD.items[module].effect.consumption.bonus
                    if(FD.items[module].effect.pollution) k_pollution += FD.items[module].effect.pollution.bonus
                    if(FD.items[module].effect.speed) k_speed += FD.items[module].effect.speed.bonus
                }
            }
            ...
            this.m_entityInfo.position.set(10, this.m_RecipeRawContainer.position.y+this.m_RecipeRawContainer.height+20)
            ...
            for (const ingredient of recipe.ingredients) {
                txt_ingredient.text += Math.round(ingredient.amount*new_crafting_speed/recipe.time*100)/100 + " /s x " + ingredient.name + "\n"
            }
            txt_ingredient.text += "\nOutput :\n"
            for (const result of recipe.results) {
                txt_ingredient.text += Math.round(result.amount*new_crafting_speed/recipe.time*(1+k_productivity)*100)/100 + " /s x " + result.name + "\n"
            }
        ...
        if(entity.entityData.type == "inserter") {
            ...
            let type = ["burner_inserter","inserter","long_handed_inserter","fast_inserter","stack_inserter","stack_filter_inserter","filter_inserter"]
