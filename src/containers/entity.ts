...
export class EntityContainer extends PIXI.Container {
    ...
    pointerDownEventHandler(e: PIXI.interaction.InteractionEvent) {
        ...
        if (e.data.button === 0) {
            if (G.currentMouseState === G.mouseStates.NONE && !G.openedGUIWindow && !keyboard.actions.copyPasteEntitySettings.pressed) {
                G.editEntityContainer.create(this.entity_number)
            }
            if (keyboard.actions.copyPasteEntitySettings.pressed) this.pasteData()
            ...
            if (keyboard.actions.copyPasteEntitySettings.pressed) {
