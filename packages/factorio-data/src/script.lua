...
local mods = { 'core', 'base' }

-- load data
for mod in list_iter(mods) do
    __MOD_NAME__ = mod
    require('data')
end
...
for mod in list_iter(mods) do
    __MOD_NAME__ = mod
    optrequire('data-updates')
end

-- load data-final-fixes
for mod in list_iter(mods) do
    __MOD_NAME__ = mod
    optrequire('data-final-fixes')
end
...
for mod in list_iter(mods) do
    __MOD_NAME__ = mod
    table_merge(locale, require('__locale__'))
end
...
do
    ...
    for proto in list_iter(placeableEntityPrototypes) do
        for _, entity in pairs(data.raw[proto]) do
...
do
    ...
    addEntriesToSubroups(data.raw.recipe)
    addEntriesToSubroups(data.raw.fluid, 'fluid')
    addEntriesToSubroups(data.raw['virtual-signal'])
...
-- UTILITY SPRITES
do
    local utilitySprites = data.raw['utility-sprites'].default
    utilitySprites.type = nil
    utilitySprites.name = nil
    output.utilitySprites = utilitySprites
end

return rapidjson.encode(output)
